...
const bool kDebugPo50 = std::getenv("MANIFOLD_DEBUG_PO50") != nullptr;
...
inline std::pair<int, vec2> Shadow01(const int a0, const int b1,
  ...
  if (kDebugPo50) {
    fprintf(stderr,
            "PO50 Shadow01 forward=%d a0=%d b1=%d s01=%d yz=(%.6f,%.6f) x=(%.6f,%.6f,%.6f) edgeX=(%.6f,%.6f)\n",
            forward, a0, b1, s01, yz01[0], yz01[1], inA.vertPos_[a0].x,
            inA.vertPos_[a0].y, inA.vertPos_[a0].z, inB.vertPos_[b1s].x,
            inB.vertPos_[b1e].x);
  }
...
struct Kernel02 {
  ...
  std::pair<int, double> operator()(int a0, int b2) {
    ...
    if (s02 == 0) {  // No intersection
      if (kDebugPo50) {
        fprintf(stderr,
                "PO50 Kernel02 forward=%d a0=%d b2=%d s02=%d k=%d (no overlap)\n",
                forward, a0, b2, s02, k);
      }
      z02 = NAN;
    } else {
      ...
      if (kDebugPo50) {
        fprintf(stderr,
                "PO50 Kernel02 forward=%d a0=%d b2=%d s02=%d k=%d z=%.6f\n",
                forward, a0, b2, s02, k, z02);
      }
    }
